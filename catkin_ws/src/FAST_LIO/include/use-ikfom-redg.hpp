#ifndef USE_IKFOM_REDG_H
#define USE_IKFOM_REDG_H
...
// [reduced-21] 只移除 ((S2, grav)):误差状态 23 → 21(S2 占 2 自由度)。
// ba 仍在状态中在线估计 —— 这是策略 B(只冻重力)的真实现。
extern Eigen::Vector3d g_const_redg;
...
Eigen::Matrix<double, 21, 1> get_f(state_ikfom &s, const input_ikfom &in)
{
	...
	for(int i = 0; i < 3; i++ ){
		res(i) = s.vel[i];
		res(i + 3) =  omega[i]; 
		res(i + 12) = a_inertial[i] + g_const_redg[i]; 
	}
	return res;
}

Eigen::Matrix<double, 21, 21> df_dx(state_ikfom &s, const input_ikfom &in)
{
	Eigen::Matrix<double, 21, 21> cov = Eigen::Matrix<double, 21, 21>::Zero();
...
Eigen::Matrix<double, 21, 12> df_dw(state_ikfom &s, const input_ikfom &in)
{
	Eigen::Matrix<double, 21, 12> cov = Eigen::Matrix<double, 21, 12>::Zero();
