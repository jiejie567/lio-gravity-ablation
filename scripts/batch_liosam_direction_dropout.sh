...
for dropout in drop2x20 drop3x20 drop5x20; do
    ROOT="results/liosam_direction_dropout/mcd_tuhh_day04_${dropout}"
    LIDAR="data/mcd/degraded/tuhh_day_04_${dropout}.bag"
        ...
        for variant in FG-BA GE-BA; do
            ...
            if [[ -f "$output/state_log.csv" && -f "$output/odometry.csv" ]] && \
               rg -q '^finished_at=' "$output/run_meta.txt"; then
                echo "skip completed ${dropout} ${variant}_${level}"
                continue
            fi
