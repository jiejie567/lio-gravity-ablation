...
ROOT=results/liosam_throughput/mcd_tuhh_day_04_rate04
...
if times["FG-BA"] != times["FG-B0"]:
    raise SystemExit(f"FAIL: full-length timestamp zero-tolerance gate: {counts}")
print(f"LIO-SAM full-length throughput gate: PASS ({counts['FG-BA']} timestamps)")
PY
