...
ROOT=results/liosam_throughput/mcd_tuhh_day_04_rate025
...
if times["FG-BA"] != times["FG-B0"]:
    raise SystemExit(f"FAIL: rate-0.25 full-length timestamp gate: {counts}")
if counts["FG-BA"] != 934:
    raise SystemExit(f"FAIL: expected 934 full-length timestamps, got {counts}")
print("LIO-SAM rate-0.25 full-length gate: PASS (934 timestamps, no resets)")
PY
