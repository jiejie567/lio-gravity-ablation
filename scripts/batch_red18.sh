#!/usr/bin/env bash
# 真降维实现: 18 维,重力与 ba 全移除(策略 C 的真实现)。降维由二进制决定,故一律以方法 A 运行
...
run() {
    ...
    [ -f "results/$seq/RED18/state_log.csv" ] && { echo "== 跳过: $seq/RED18"; return 0; }
    echo "== 开始: $seq/RED18  $(date '+%H:%M:%S')"
    ./scripts/run_experiment.sh A "$bag" -c "$cfg" -x configs/exp_tuning.yaml \
        -n RED18 -L mapping_exp_red -r "$rate" -p \
        && echo "== 完成: $seq/RED18  $(date '+%H:%M:%S')" || echo "== 失败: $seq/RED18"
}
