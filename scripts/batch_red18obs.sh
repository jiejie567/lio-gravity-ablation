#!/usr/bin/env bash
# 递减链的最后一档: 18 维真降维 + IMU 更新权重降万分之一(过程噪声 ×10^3~10^4)。
# 此时 IMU 只剩去畸变与迭代初值 —— 在同一配准核内能做到的"最不用 IMU"。降维由二进制决定,故一律以方法 A 运行
...
run() {
    ...
    [ -f "results/$seq/RED18OBS/state_log.csv" ] && { echo "== 跳过: $seq/RED18OBS"; return 0; }
    echo "== 开始: $seq/RED18OBS  $(date '+%H:%M:%S')"
    ./scripts/run_experiment.sh A "$bag" -c "$cfg" -x configs/exp_tuning_obsonly.yaml \
        -n RED18OBS -L mapping_exp_red -r "$rate" -p \
        && echo "== 完成: $seq/RED18OBS  $(date '+%H:%M:%S')" || echo "== 失败: $seq/RED18OBS"
}
