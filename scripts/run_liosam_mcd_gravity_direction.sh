...
if [[ $# -lt 6 || $# -gt 9 ]]; then
    echo "usage: $0 <FG-BA|GE-BA> <atv|handheld> <lidar-bag> <imu-bag> <output-dir> <direction-sigma-rad; <=0 disables> [start-s] [duration-s] [rate]" >&2
    exit 2
fi
...
case "$VARIANT" in
    ...
    *) echo "direction-factor ablation only admits FG-BA or GE-BA" >&2; exit 2 ;;
esac
...
CONTAINER_NAME="liosam_mcd_gravity_direction_$$"
cleanup()
{
    set +e
...
docker run --rm --name "$CONTAINER_NAME" \
    ...
    "$SETUP" "$START_S" "$DURATION_S" "$RATE" "$DIRECTION_SIGMA" \
    "$DIRECTION_LOG"
